...
class CertificateGenerator {
    ...
    bindEvents() {
        ...
        const inputs = [this.elements.fullName];
    ...
    validateForm() {
        ...
        // Security validation
        if (!this.validateInputSecurity(name)) {
        ...
        // Club name not required for this certificate
    ...
    showHelp(input) {
        const helpId = input.id === 'fullName' ? 'nameHelp' : null;
    ...
    generateFileName() {
        ...
        const safeFileName = `EndPolio_${sanitizedName}_${timestamp}.pdf`;
    ...
    async createCanvasWithTemplate() {
        ...
        // Set font properties to match CSS (.name-overlay uses bold Times New Roman)
        ctx.font = `700 ${nameFontSize}px "Times New Roman", Times, serif`;
        ctx.fillStyle = '#000000';
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        
        // Match CSS position: .name-overlay { top: 41%; left: 8% }
        const nameY = canvas.height * 0.41; // 41% from top
        const nameX = canvas.width * 0.08;  // 8% from left
        ...
        // Draw club name text
        // No club name for participant certificate
    ...
    createFallbackCanvas() {
        ...
        if (name) {
            const fontSize = Math.min(16, 800 / name.length * 3);
            ctx.font = `700 ${fontSize}px "Times New Roman", Times, serif`;
            ctx.fillStyle = '#000000';
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            ctx.fillText(name, canvas.width * 0.08, canvas.height * 0.41);
        }
